...
export default function FilterModal() {
  ...
  // Shared value for opacity and translateY
  ...
  useEffect(() => {
    if (modalVisible) {
      ...
      modalOpacity.value = withTiming(1, { duration: 300, easing: Easing.out(Easing.exp) });
      modalTranslateY.value = withTiming(0, { duration: 300, easing: Easing.out(Easing.exp) });
    } else {
      // Animate modal when it closes
      modalOpacity.value = withTiming(0, { duration: 300, easing: Easing.in(Easing.exp) });
      modalTranslateY.value = withTiming(50, { duration: 300, easing: Easing.in(Easing.exp) });
    }
  }, [modalVisible]);
          ...
          <Animated.View style={[styles.modalView, animatedStyle]}>
            ...
            <View style={styles.optionsRow}>
              {['Delivery', 'Pick Up', 'Offer', 'Online payment available'].map((offer, index) => (
                <TouchableOpacity
                  key={index}
                  style={[styles.optionButton, selectedOffer === offer && styles.selectedButton]}
                  onPress={() => setSelectedOffer(offer)}
                >
                  <Text style={[styles.optionText, selectedOffer === offer && styles.selectedText]}>
                    {offer}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            <View style={styles.optionsRow}>
              {['10-15 min', '20 min', '30 min'].map((time, index) => (
                <TouchableOpacity
                  key={index}
                  style={[styles.optionButton, selectedTime === time && styles.selectedButton]}
            ...
            <View style={styles.optionsRow}>
              {['Low To High', 'High to Low'].map((price, index) => (
                <TouchableOpacity
                  key={index}
                  style={[styles.optionButton, selectedPricing === price && styles.selectedButton]}
                  onPress={() => setSelectedPricing(price)}
                >
                  <Text style={[styles.optionText, selectedPricing === price && styles.selectedText]}>
                    {price}
                  </Text>
                </TouchableOpacity>
              ))}
